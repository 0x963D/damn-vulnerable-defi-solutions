...
describe("[Challenge] Puppet v3", function () {
  ...
  const hacker_INITIAL_TOKEN_BALANCE = 110n * 10n ** 18n;
  ...
  before(async function () {
    ...
    // Initialize hacker account
    // using private key of account #2 in Hardhat's node
    hacker = new ethers.Wallet(
    ...
    // Setup initial token balances of lending pool and hacker
    ...
    // Ensure hacker doesn't have that much ETH
    expect(await ethers.provider.getBalance(hacker.address)).to.be.lt(
  ...
  after(async function () {
    ...
    // hacker has taken all tokens out of the pool
