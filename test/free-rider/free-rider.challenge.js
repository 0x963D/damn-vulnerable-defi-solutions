...
describe("[Challenge] Free Rider", function () {
  ...
  before(async function () {
    ...
    [deployer, hacker, devs] = await ethers.getSigners();

    // hacker starts with limited ETH balance
    ...
    // Deploy devs' contract, adding the hacker as the beneficiary
    devsContract = await (
      await ethers.getContractFactory("FreeRiderRecovery", devs)
    ).deploy(
      hacker.address, // beneficiary
      nft.address,
      { value: BOUNTY }
    );
  });
  ...
  after(async function () {
    ...
    // hacker must have earned all ETH
    expect(await ethers.provider.getBalance(hacker.address)).to.be.gt(BOUNTY);
