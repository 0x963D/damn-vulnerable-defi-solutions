...
describe("[Challenge] ABI smuggling", function () {
  ...
  before(async function () {
    ...
    [deployer, hacker, recovery] = await ethers.getSigners();
    ...
    const hackerPermission = await vault.getActionId(
      "0xd9caed12",
      hacker.address,
      vault.address
    );
    await vault.setPermissions([deployerPermission, hackerPermission]);
    expect(await vault.permissions(deployerPermission)).to.be.true;
    expect(await vault.permissions(hackerPermission)).to.be.true;
    ...
    expect(await token.balanceOf(hacker.address)).to.eq(0);
    ...
    await expect(
      vault.connect(hacker).withdraw(token.address, hacker.address, 10n ** 18n)
    ).to.be.revertedWithCustomError(vault, "CallerNotAllowed");
  });
