...
describe("[Challenge] Wallet mining", function () {
  ...
  before(async function () {
    ...
    [deployer, ward, hacker] = await ethers.getSigners();
    ...
    expect(await authorizer.can(hacker.address, DEPOSIT_ADDRESS)).to.be.false;
    ...
    await expect(
      walletDeployer.can(hacker.address, DEPOSIT_ADDRESS)
    ).to.be.reverted;
    ...
    expect(await token.balanceOf(hacker.address)).eq(0);
  });
