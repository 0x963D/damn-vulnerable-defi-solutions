...
describe("[Challenge] Puppet", function () {
  ...
  const HACKER_INITIAL_TOKEN_BALANCE = 1000n * 10n ** 18n;
  const HACKER_INITIAL_ETH_BALANCE = 25n * 10n ** 18n;
  ...
  before(async function () {
    ...
    setBalance(hacker.address, HACKER_INITIAL_ETH_BALANCE);
    ...
    // Setup initial token balances of pool and hacker accounts
    await token.transfer(hacker.address, HACKER_INITIAL_TOKEN_BALANCE);
  ...
  it("Exploit", async function () {
    ...
    const Exploit = await ethers.getContractFactory("PuppetExploit", hacker);
    ...
    await token
      .connect(hacker)
      .transfer(exploit.address, HACKER_INITIAL_TOKEN_BALANCE);

    await exploit.connect(hacker).exploit();
  });

  after(async function () {
    /** SUCCESS CONDITIONS - NO NEED TO CHANGE ANYTHING HERE */
    // hacker executed a single transaction // couldn't do it in one transaction
    expect(await ethers.provider.getTransactionCount(hacker.address)).to.eq(6);
    ...
    expect(await token.balanceOf(hacker.address)).to.be.gte(
      POOL_INITIAL_TOKEN_BALANCE,
      "Not enough token balance in hacker"
    );
  });
});
