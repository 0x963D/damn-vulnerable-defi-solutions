...
describe("[Challenge] The rewarder", function () {
  ...
  let users, deployer, alice, bob, charlie, david, hacker;
  let liquidityToken, flashLoanPool, rewarderPool, rewardToken, accountingToken;

  before(async function () {
    /** SETUP SCENARIO - NO NEED TO CHANGE ANYTHING HERE */

    [deployer, alice, bob, charlie, david, hacker] = await ethers.getSigners();
    ...
    // hacker starts with zero DVT tokens in balance
    expect(await liquidityToken.balanceOf(hacker.address)).to.eq(0);
  ...
  after(async function () {
    ...
    // Rewards must have been issued to the hacker account
    expect(await rewardToken.totalSupply()).to.be.gt(
      await rewarderPool.REWARDS()
    );
    const hackerRewards = await rewardToken.balanceOf(hacker.address);
    expect(hackerRewards).to.be.gt(0);

    // The amount of rewards earned should be close to total available amount
    const delta = (await rewarderPool.REWARDS()).sub(hackerRewards);
    expect(delta).to.be.lt(10n ** 17n);

    // Balance of DVT tokens in hacker and lending pool hasn't changed
    expect(await liquidityToken.balanceOf(hacker.address)).to.eq(0);
